...
serve(async (req: any) => {
  ...
  try {
    ...
    if (!targetId) {
      console.warn('update-company missing target id', { body });
      return jsonResponse({ error: 'company_id (or id) is required' }, 400);
    }
    ...
    if (callerUserId) {
      ...
      if (
        ...
        String(callerProfile.company) !== String(company.id)
      ) {
    ...
    if (!upd.ok) return jsonResponse({ error: 'failed_to_update_company', details: upd.data }, 500);
