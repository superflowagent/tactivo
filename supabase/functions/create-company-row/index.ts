...
serve(async (req: any) => {
  ...
  try {
    ...
    if (!table || !name || !company_id)
      return jsonResponse({ error: 'table, name and company_id are required' }, 400);

    if (!['anatomy', 'equipment'].includes(table))
      return jsonResponse({ error: 'invalid_table' }, 400);
    ...
    if (!insertResp.ok) {
      return jsonResponse(
        { error: 'insert_failed', status: insertResp.status, details: insertJson },
        500
      );
    }
