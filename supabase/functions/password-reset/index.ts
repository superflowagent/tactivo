...
serve(async (req) => {
  try {
    ...
    const { data, error } = await supabase.auth.verifyOtp({ token: tokenHash, type: 'recovery' } as any)
    if (error) {
      // On failure, redirect to client with an error query so UI shows message
      const errUrl = `${APP_URL.replace(/\/$/, '')}/auth/password-reset?error=${encodeURIComponent(error.message || 'verify_failed')}`
      return Response.redirect(errUrl, 302)
    }
