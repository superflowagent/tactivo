...
serve(async (req) => {
    try {
        ...
        try {
            const res = await supabase.auth.verifyOtp({ token: tokenHash, type: 'recovery' } as any)
            data = res.data
            error = res.error
        } catch (e) {
        ...
        if (error) {
            try {
                const res2 = await supabase.auth.verifyOtp({ token_hash: tokenHash, type: 'recovery' } as any)
                data = res2.data
                error = res2.error
            } catch (e) {
        ...
        if (error && providedEmail) {
            try {
                const res3 = await supabase.auth.verifyOtp({ email: providedEmail, token: tokenHash, type: 'recovery' } as any)
                data = res3.data
                error = res3.error
            } catch (e) {
        ...
        if (error) {
            ...
            const errPayload = encodeURIComponent(JSON.stringify({ message: error.message || 'verify_failed', details: error }))
        ...
        try { console.warn('password-reset exchange error', String(e?.message || e)) } catch { /* ignore */ }
        const APP_URL = Deno.env.get('APP_URL') || (req.headers.get('origin') ?? '')
