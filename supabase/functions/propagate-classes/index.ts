...
serve(async (req: any) => {
    ...
    try {
        ...
        if (!insertRes.ok) return jsonResponse({ error: 'failed_to_insert_events', details: insertRes }, 500);
        ...
        return jsonResponse({ ok: true, inserted: inserted.length, client_counts: clientCounts, clients_updated: clientsUpdated }, 200);
    } catch (err: any) {
