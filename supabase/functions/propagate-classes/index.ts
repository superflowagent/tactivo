...
serve(async (req)=>{
  ...
  try {
    ...
    if (!month || !year) return jsonResponse({
      error: 'month and year are required'
    }, 400);
    if (!templates || !Array.isArray(templates) || templates.length === 0) return jsonResponse({
      ...
      for(let day = 1; day <= daysInMonth; day++){
        ...
        if (currentDate.getDay() === templateDay) {
          ...
          eventsToCreate.push({
            ...
            client: Array.isArray(t.client) ? t.client.length ? t.client : null : t.client ? [
              t.client
            ] : null,
            professional: Array.isArray(t.professional) ? t.professional.length ? t.professional : null : t.professional ? [
              t.professional
            ] : null,
            company,
    ...
    if (eventsToCreate.length === 0) return jsonResponse({
      ok: true,
      inserted: 0
    }, 200);
