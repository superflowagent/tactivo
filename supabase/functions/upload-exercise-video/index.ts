...
serve(async (req: any) => {
    ...
    try {
        ...
        if (!bucket || !filename || !content_b64 || !content_type || (!body.root && (!company_id || !exercise_id))) {
            return jsonResponse({ error: 'bucket, filename, content_b64, content_type and (exercise_id and company_id when root not set) are required' }, 400);
        }
        ...
        if (!uploadResp.ok) {
            return jsonResponse({ error: 'upload_failed', status: uploadResp.status, details: uploadJson }, 500);
        }
