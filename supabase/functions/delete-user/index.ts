...
serve(async (req)=>{
  ...
  try {
    ...
    if (!profile_id) return jsonResponse({
      error: 'profile_id is required'
    }, 400);
    // Lookup profile
    const prof = await restProfiles('GET', undefined, `id=eq.${profile_id}`);
    if (!prof.ok || !prof.data || Array.isArray(prof.data) && prof.data.length === 0) return jsonResponse({
