...
serve(async (req) => {
    ...
    try {
        ...
        if (!authHeader || !authHeader.toLowerCase().startsWith('bearer ')) {
            return jsonResponse({ error: 'Unauthorized', auth_debug: { authorization_present: !!authHeader } }, 401)
        }
        ...
        if (!userResp.ok) {
            ...
            try { authErr = await userResp.json() } catch { authErr = null }
        ...
        if (!prof.ok || !prof.data || (Array.isArray(prof.data) && prof.data.length === 0)) return jsonResponse({ error: 'profile_not_found' }, 404)
        ...
        try {
            ...
            if ((callerProfile.role || '') !== 'professional' || String(callerProfile.company) !== String(profile.company)) {
        ...
        if (authUserId) {
            try {
                const delUserResp = await fetch(`${SUPABASE_URL.replace(/\/$/, '')}/auth/v1/admin/users/${authUserId}`, {
                    method: 'DELETE',
                    headers: {
                        'Content-Type': 'application/json',
                        'apikey': SUPABASE_SERVICE_ROLE_KEY,
                ...
                let json: any = null
                try { if (text) json = JSON.parse(text) } catch { json = text }
        ...
        return jsonResponse({ ok: true, profile_id, auth_user: authUserId, deletedAuth, deletedProfile }, 200)
    } catch (err: any) {
