...
serve(async (req) => {
  ...
  try {
    ...
    const APP_URL =
      Deno.env.get('APP_URL') || reqOrigin || Deno.env.get('VITE_SUPABASE_URL') || SUPABASE_URL;
    ...
    console.info('send-invite called', { method: req.method, origin: req.headers.get('origin') });
    ...
    if (!provided) {
      ...
      if (!userResp.ok) {
        ...
        if (authErr) {
          ...
          else if (typeof authErr === 'object')
            authErrMsg = authErr.message || authErr.error || JSON.stringify(authErr);
        }
    ...
    if (!profile_id && !email)
      return new Response(JSON.stringify({ error: 'profile_id or email is required' }), {
        status: 400,
      });
    ...
    if (profile_id) {
      ...
      const { ok, data } = await restProfiles(
        'GET',
        undefined,
    ...
    const expiresAt = new Date(Date.now() + days * 24 * 60 * 60 * 1000)
      .toISOString()
      .replace('Z', ''); // keep timestamp without tz if desired

    // Update profile with token & expiry (service role)
    const upd = await restProfiles(
      'PATCH',
      ...
      if (cuResp.ok && cuJson) {
        ...
        if (newUserId) {
          ...
          for (let attempt = 0; attempt < 3; attempt++) {
            try {
              const patchRes = await restProfiles(
                'PATCH',
          ...
          console.warn('send-invite: user created but no id in response', { body: cuJson });
        }
      }
    } catch (e: any) {
    ...
    try {
      ...
      const m = raw.match(
        /(https?:\/\/[^\s"']+\/functions\/v1\/password-reset\?access_token=[^&\s"']+)/
      );
    ...
    return jsonResponse(
      { ok: true, invite_link, resetUrl, email_link, createUserResult, sendResult },
      200
    );
  } catch (err: any) {
