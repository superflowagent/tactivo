...
serve(async (req: any) => {
  ...
  try {
    ...
    if (callerUserId) {
      ...
      if (!(callerProfile.role === 'professional' || callerProfile.role === 'admin'))
        return jsonResponse({ error: 'forbidden_role' }, 403);
      if (String(callerProfile.company) !== String(company))
        return jsonResponse({ error: 'forbidden_company' }, 403);
    }
