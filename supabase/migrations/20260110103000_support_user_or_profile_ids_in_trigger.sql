-- Migration: support client ids that are either profile.id or profiles.user (auth.user id)
...
DECLARE
BEGIN
  -- Helper: normalize NEW.client / OLD.client into table of uuids (client_ids)
  -- INSERT
  IF TG_OP = 'INSERT' THEN
    IF COALESCE(NEW.type, '') = 'class' AND NEW.client IS NOT NULL THEN
      WITH client_ids AS (
        SELECT unnest( (COALESCE(NEW.client, ARRAY[NEW.client]::uuid[]))::uuid[] ) AS id
      )
  ...
  IF TG_OP = 'DELETE' THEN
    IF COALESCE(OLD.type, '') = 'class' AND OLD.client IS NOT NULL THEN
      WITH client_ids AS (
        SELECT unnest( (COALESCE(OLD.client, ARRAY[OLD.client]::uuid[]))::uuid[] ) AS id
      )
      UPDATE public.profiles p
      SET class_credits = COALESCE(class_credits, 0) + 1
      WHERE (p.id IN (SELECT id FROM client_ids) OR p.user IN (SELECT id FROM client_ids))
