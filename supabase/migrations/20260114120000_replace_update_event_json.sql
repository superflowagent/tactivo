-- Migration: replace update_event_json to robustly parse JSON `changes` payload
-- - Accept `client` and `professional` as JSON arrays or strings (uuid)
-- - Update only fields present in `changes`
-- - Provide clear errors for invalid uuid values

-- Drop existing function first if it exists (helps when return type changed)
...
BEGIN
  ...
  SELECT company, type, client INTO v_event_company, v_event_type, v_event_client FROM public.events WHERE id = v_id;
  ...
  -- auth.uid() returns NULL/'' when executed in the SQL editor; cast only when non-empty
