-- Normalize nested SELECT wrappers around auth.*() that some policies still contain
BEGIN;
...
BEGIN
  FOR r IN
    ...
    FROM pg_policy
    WHERE (pg_get_expr(polqual, polrelid) ~ 'SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?uid"?\(\)' OR pg_get_expr(polwithcheck, polrelid) ~ 'SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?uid"?\(')
  LOOP
    ...
    IF using_expr IS NOT NULL THEN
      ...
      -- collapse nested SELECT wrappers
      new_using := regexp_replace(new_using, '\(\s*SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?uid"?\(\)\s*AS\s*"?uid"?\s*\)\s*AS\s*"?uid"?\s*\)', '(select auth.uid())::uuid','gi');
      new_using := regexp_replace(new_using, '\(\s*SELECT\s*"?auth"?\.?"?uid"?\(\)\s*AS\s*"?uid"?\s*\)', '(select auth.uid())::uuid','gi');
      new_using := regexp_replace(new_using, '\(\s*SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?role"?\(\)\s*AS\s*"?role"?\s*\)\s*AS\s*"?role"?\s*\)', '(select auth.role())','gi');
    ...
    IF check_expr IS NOT NULL THEN
      ...
      new_check := regexp_replace(new_check, '\(\s*SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?uid"?\(\)\s*AS\s*"?uid"?\s*\)\s*AS\s*"?uid"?\s*\)', '(select auth.uid())::uuid','gi');
      new_check := regexp_replace(new_check, '\(\s*SELECT\s*"?auth"?\.?"?uid"?\(\)\s*AS\s*"?uid"?\s*\)', '(select auth.uid())::uuid','gi');
      new_check := regexp_replace(new_check, '\(\s*SELECT\s*\(\s*SELECT\s*"?auth"?\.?"?role"?\(\)\s*AS\s*"?role"?\s*\)\s*AS\s*"?role"?\s*\)', '(select auth.role())','gi');
    ...
    IF new_using IS NOT NULL OR new_check IS NOT NULL THEN
      ...
      RAISE NOTICE 'Normalizing nested auth selects for % on %', r.polname, r.relname;
