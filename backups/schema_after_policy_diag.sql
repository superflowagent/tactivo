...
CREATE INDEX "idx_classes_templates_company1" ON "public"."classes_templates" USING "btree" ("company");
...
CREATE POLICY "Enable insert for authenticated users only" ON "public"."profiles" TO "authenticated" WITH CHECK ((("user" = ( SELECT "auth"."uid"() AS "uid")) OR ("user" IS NULL)));
...
CREATE POLICY "combined_anatomy_ALL_24c868726485fb7f398a79147c9c1ea9" ON "public"."anatomy" TO "authenticated", "service_role" USING (((( SELECT "auth"."role"() AS "role") = 'service_role'::"text") OR (EXISTS ( SELECT 1
   FROM "public"."profiles" "p"
...
CREATE POLICY "combined_profiles_SELECT_c43d1ed0fe3a9e43d3702f4bc9deb03c" ON "public"."profiles" FOR SELECT TO "authenticated" USING (((( SELECT "auth"."uid"() AS "uid") = "user") OR (( SELECT "auth"."uid"() AS "uid") = "id") OR "public"."is_same_company"("company") OR ((( SELECT "auth"."uid"() AS "uid") = "user") OR "public"."is_profile_member_of"("company"))));
