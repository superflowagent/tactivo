...
async function adminAuth() {
    ...
    // Prefer admin auth endpoint if available (supports superusers stored in _superusers)
    try {
        ...
        if (ar.ok) {
            ...
            adminToken = aj.token
            adminTokenFetchedAt = Date.now()
            console.info('Acquired admin token via admins auth')
            return
        } else if (ar.status !== 404) {
            const txt = await ar.text()
            throw new Error(`admin auth (admins endpoint) failed: ${ar.status} ${txt}`)
        }
        // if 404, fallthrough to try users auth
    } catch (err) {
        // If fetch itself failed, log and continue to try users auth
        console.warn('admins auth attempt failed, falling back to users auth:', err.message)
    }
    ...
    if (!res.ok) {
        ...
        throw new Error(`admin auth (users endpoint) failed: ${res.status} ${txt}`)
    }
