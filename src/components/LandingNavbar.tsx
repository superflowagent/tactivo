    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-background/70 backdrop-blur-md border-b border-muted">
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-6">
          {links.map((l) => (
            <a key={l.title} href={l.href} className="text-neutral-900 hover:text-primary transition-colors">
              {l.title}
            </a>
          ))}
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <a href="/login" className="p-[3px] relative rounded-[6px] overflow-hidden" aria-label="Iniciar sesión">
            <div className="absolute inset-0 rounded-[6px]" style={{ background: 'var(--primary-gradient)' }} aria-hidden />
      ...
      {open && (
        <div className="md:hidden border-t border-muted bg-background/95">
          <div className="px-6 py-4 flex flex-col gap-3">
            ...
            <a href="/login" className="text-primary transition-colors mt-2">Iniciar sesión</a>
          </div>
        </div>
      )}
    </nav>
