    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (exercise) {
                ...
                try {
                    const { data: updatedRow, error: fetchErr } = await supabase.from('exercises').select('*').eq('id', exercise.id).maybeSingle();
                    if (!fetchErr && updatedRow) {
                        try { window.dispatchEvent(new CustomEvent('exercise-updated', { detail: updatedRow })); } catch { }
                    }
                } catch { /* non-fatal */ }
                ...
                try { window.dispatchEvent(new CustomEvent('exercise-updated', { detail: newEx })); } catch { }
