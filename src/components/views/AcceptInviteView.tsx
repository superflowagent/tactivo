  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      if (!sessionData?.session) {
        setMessage(
          'Debes iniciar sesión o completar el restablecimiento de contraseña enviado por email antes de aceptar la invitación. Revisa tu correo.'
        );
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="max-w-2xl mx-auto mt-20 p-6">
      ...
      <div className="flex gap-2">
        <Button onClick={handleAccept} disabled={loading || !token}>
      ...
      <div className="mt-6 text-sm text-muted-foreground">
        Si aún no has establecido tu contraseña, revisa tu correo y utiliza el link de
        restablecimiento; una vez inicies sesión vuelve aquí y pulsa "Aceptar invitación".
      </div>
    </div>
  );
}
