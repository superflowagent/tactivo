...
export default function CalendarioList({ events, onRowClick, onDeleteComplete, canDelete = true }: Props) {
    ...
    const handleBulkDeleteConfirm = async () => {
        ...
        try {
            ...
            const failed = results.filter((r: any) => r.status === 'rejected' || (r.status === 'fulfilled' && r.value?.error));
            if (failed.length > 0) {
                ...
                <Table>
                    <TableHeader>
                        <TableRow>
                            <TableHead className="w-12">
                                {canDelete ? (
                                    (() => {
                                        const allSelected = futureEvents.length > 0 && futureEvents.every((e) => selectedIds.has(e.id));
                    ...
                    <TableBody>
                        {pageItems.map((ev) => {
                            ...
                            const dateStr = start.toLocaleDateString('es-ES', { year: 'numeric', month: 'short', day: 'numeric' });
                            ...
                            return (
                                <TableRow key={ev.id} className="hover:bg-muted">
                                    ...
                                    <TableCell className="cursor-pointer" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{dateStr}</TableCell>
                                    <TableCell className="cursor-pointer" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{timeStr}</TableCell>
                                    <TableCell className="cursor-pointer capitalize" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{type}</TableCell>
                                    <TableCell className="cursor-pointer font-medium" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{ev.title}</TableCell>
                                    <TableCell className="cursor-pointer" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{pros}</TableCell>
                                    <TableCell className="cursor-pointer" onClick={() => onRowClick && onRowClick({ event: { id: ev.id } })}>{clients}</TableCell>
            ...
            <AlertDialog open={bulkDeleteDialogOpen} onOpenChange={setBulkDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>Se eliminarán {selectedCount} eventos. Esta acción no se puede deshacer.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
