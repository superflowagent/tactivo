...
export const getPublicUrl = (bucket: string, path: string) => {
    try {
        ...
        return supabaseUrl ? `${supabaseUrl.replace(/\/$/, "")}/storage/v1/object/public/${b}/${p}` : null;
    } catch {
        return null;
    }
};
    ...
    try {
        ...
        if ((toUpload as File).size > directUploadAbove) {
            try {
                ...
                const body: any = { bucket, filename: filenameOnly, content_b64, content_type: (toUpload as File).type || 'application/octet-stream' };
        ...
        if (error && (errMsg.includes('row-level security') || errMsg.includes('violates') || (error?.status === 403) || (error?.statusCode === '403'))) {
            ...
            try {
                ...
                const fnUrl = `${supabaseUrl.replace(/\/$/, '')}/functions/v1/upload-exercise-video`;
                const body = { bucket, company_id: company, exercise_id: exerciseId, filename: filenameOnly, content_b64, content_type: (toUpload as File).type || 'application/octet-stream' } as any;
