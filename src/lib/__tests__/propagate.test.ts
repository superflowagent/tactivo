...
describe('propagateClasses', () => {
  ...
  it('succeeds when function returns ok', async () => {
    ...
    const res = await propagateClasses('c1', 2, 2026, [
      { id: 't1', day: 1, time: '10:00', duration: 60, client: ['u1'] },
    ]);
  ...
  it('throws when session invalid', async () => {
    ...
    await expect(() => propagateClasses('c1', 2, 2026, [{ id: 't1' }])).rejects.toThrow(
      'invalid_session'
    );
  });
  ...
  it('throws when templates missing or empty', async () => {
    ...
    await expect(() => propagateClasses('c1', 2, 2026, null as any)).rejects.toThrow(
      'missing_templates'
    );
  });
});
