...
(async () => {
  try {
    ...
    const profileRes = await fetch(
      `${SUPABASE_URL.replace(/\/$/, '')}/rest/v1/profiles?select=user_id,email&id=eq.${encodeURIComponent(email)}`
    , {
      headers: {
    ...
    if (Array.isArray(profiles) && profiles.length > 0) {
      // profile may have user_id or id depending on schema
      userId = profiles[0].user_id || profiles[0].id || null;
    }
