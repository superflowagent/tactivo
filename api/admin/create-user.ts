...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        const tmpPassword = Math.random().toString(36).slice(2, 12) + Math.random().toString(36).slice(2, 12)
        let createRes: any
        try {
            ...
            if (msg.includes('already exists')) {
                ...
                const { data: userData } = await supabase.from('profiles').select('user_id').eq('email', email).maybeSingle()
                if (userData?.user_id) {
        ...
        let upErr: any = null
        try {
            const { error } = await supabase.from('profiles').upsert(profilePayloadUserId, { onConflict: 'user_id' })
            upErr = error
        } catch (e: any) {
        ...
        if (upErr) {
            ...
            const { error: idErr } = await supabase.from('profiles').upsert(profilePayloadId, { onConflict: 'id' })
            if (idErr) throw idErr
        }

        // Send reset link
        const redirect = redirectTo || (process.env.RESET_REDIRECT_URL || (process.env.VITE_SUPABASE_URL ? `${process.env.VITE_SUPABASE_URL}/auth/password-reset` : undefined))
