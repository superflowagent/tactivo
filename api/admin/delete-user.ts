  ...
  try {
    ...
    if (!provided) {
      ...
      const { data: callerProfile } = await supabase
        .from('profiles')
        .select('id, user, role, company')
        .eq('user', callerUserId)
        .maybeSingle();
      ...
      if (String(callerProfile.company) !== String(targetProfile.company))
        return res.status(403).json({ error: 'forbidden' });
    } else {
    ...
    try {
      if (targetProfile.photo_path) {
        ...
        const keysToTry = [
          targetProfile.photo_path,
          `${targetProfile.id}/${targetProfile.photo_path}`,
        ];
